...
from experiments import attention_allocation_experiment, attention_allocation_experiment_plotting
from ml_gym.agents import allocation_agents, random_agents
from ml_gym.environments import attention_allocation

FLAGS = flags.FLAGS
flags.DEFINE_string("output_dir", "/tmp/", "Output directory to write results to.")
...
def _print_discovered_missed_incidents_report(value, report):
    ...
    missed_incidents = np.array(report["metrics"]["occurred_incidents"]) - np.array(
    ...
    print(
        "REPORT dynamic_value: {}\ndiscovered_total: {}\nmissed_total: {}\ndiscovered_locations: {}\nmissed_locations: {}\n".format(
            value, discovered_total, missed_total, discovered_incidents, missed_incidents
        )
    )
...
def mle_greedy_agent_resource_all_dynamics():
    ...
    experiment = _setup_experiment()
    experiment.agent_class = allocation_agents.MLEGreedyAgent
    experiment.agent_params = allocation_agents.MLEGreedyAgentParams(burn_steps=25, window=100)
...
def main(argv):
    ...
    agent_names = [
        ...
        "greedy alpha=0.75",
        "uniform",
        "proportional epsilon=0.1",
        "proportional epsilon=0.5",
    ]
    dataframe = attention_allocation_experiment_plotting.create_dataframe_from_results(
        agent_names,
        [greedy_reports, greedy_fair_reports, uniform_reports, mle1_reports, mle5_reports],
    )
    ...
    attention_allocation_experiment_plotting.plot_discovered_missed_clusters(
        loc_dataframe, os.path.join(FLAGS.output_dir, "dynamic_rate_across_agents_locations")
    )
    ...
    attention_allocation_experiment_plotting.plot_discovered_occurred_ratio_locations(
        loc_dataframe, os.path.join(FLAGS.output_dir, "discovered_to_occurred_locations")
    )
    ...
    attention_allocation_experiment_plotting.plot_occurence_action_single_dynamic(
        ...
        os.path.join(FLAGS.output_dir, "proportional_incidents_actions_over_time"),
    )
