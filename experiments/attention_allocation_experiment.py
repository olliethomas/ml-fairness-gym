...
class RatioMetric(core.Metric):
    ...
    def __init__(self, env, selection_fn, realign_fn=None):
...
class WeightedRatioMetric(core.Metric):
    ...
    def measure(self, env):
        ...
        numerator = np.sum(
            np.asarray([self.selection_fn(history_item)[0] for history_item in history]), axis=0
        )
        denominator = np.sum(
            np.asarray(
                [self.selection_fn(history_item)[1] for history_item in history], dtype=float
            ),
            axis=0,
        )
...
def run_single_simulation(experiment):
    ...
    discovered_occurred_ratio_metric = RatioMetric(env, _discovered_occurred_selection_fn)
    ...
    metric_results = run_util.run_simulation(
        env, agent, metrics, experiment.num_steps, experiment.seed
    )
...
def run(experiment):
    ...
    experiments = [experiment_item for experiment_item in run_generator(copy.copy(experiment))]
