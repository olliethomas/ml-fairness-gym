...
from ml_gym.metrics import error_metrics, value_tracking_metrics
...
class CollegeExperiment(object):
    ...
    """
        realign_variables = ["test_scores_x", "applicant_groups", "true_eligible", "params"]
        realigned_history = []
        for (state, _), (next_state, next_action) in more_itertools.pairwise(history):
    ...
    def accuracy_nr_fn(self, step):
        ...
        return [
            pred == groundtruth
            for pred, groundtruth in zip(step.state.selected_applicants, step.state.true_eligible)
        ]

    def selection_fn_social_burden_eligible_auditor(self, step):
        """This returns individual burden of only eligible candidates."""
        return np.array(step.state.true_eligible) * np.array(step.state.individual_burden)
    ...
    def run_experiment(self):
        ...
        overall_accuracy = error_metrics.AccuracyMetric(
            ...
            stratify_fn=lambda x: [1 for _ in range(env.initial_params.num_applicants)],
            realign_fn=self.realign_history,
        )
        ...
        final_threshold = value_tracking_metrics.FinalValueMetric(
            env=env, state_var="decision_threshold", realign_fn=self.realign_history
        )
        ...
        return core.to_json({"metric_results": dict(zip(metric_names, metric_results)),})
