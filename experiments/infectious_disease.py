...
import core
import rewards
from ml_gym.agents import infectious_disease_agents
from ml_gym.environments import infectious_disease

GRAPHS = {"karate": nx.karate_club_graph(), "chain": nx.generators.path_graph(12)}


class DayTracker(core.Metric):
    """Counts the number of individuals in a health state every step."""

    def __init__(self, env, health_state):
    ...
    def measure(self, env):
        ...
        return [
            np.count_nonzero(
                np.array(step.state.asdict()["health_states"]) == self.health_state_idx
            )
            for step in history
        ]
...
class RandomAgent(infectious_disease_agents._BaseAgent):  # pylint: disable=protected-access
    ...
    def _triage(self, observation):
        ...
        triage_scores = np.array([self.rng.rand() for _ in observation["health_states"]])
...
class Experiment(core.Params):
    ...
    def scenario_builder(self):
        ...
        env = infectious_disease.build_sir_model(
            ...
            # get sick.
            treatment_transition_matrix=np.array([[0, 0, 1], [0, 1, 0], [0, 0, 1]]),
    ...
    def run(self):
        ...
        patient0 = env.state.rng.choice(len(env.initial_params.initial_health_state))
        initial_health_state[patient0] = 1
        env.set_initial_health_state(initial_health_state)

        metrics = {"states": StateTracker(env), "sick-days": DayTracker(env, health_state=1)}
        ...
        for _ in tqdm.trange(self.num_steps):
            ...
            assert not done, (
                "The environment ended the simulation before the " "experiment was over."
            )
        metric_results = {name: metric.measure(env) for name, metric in metrics.items()}
