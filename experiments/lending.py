...
import rewards
import run_util
from ml_gym.agents import classifier_agents, oracle_lending_agent, threshold_policies
from ml_gym.environments import lending, lending_params
from ml_gym.metrics import error_metrics, lending_metrics, value_tracking_metrics
...
class Experiment(core.Params):
    ...
    def scenario_builder(self):
        ...
        agent_params = classifier_agents.ScoringAgentParams(
            ...
            cost_matrix=params.CostMatrix(fn=0, fp=-1, tp=env_params.interest_rate, tn=0),
        )
    ...
    """
        ...
        metrics = {
            "initial_credit_distribution": lending_metrics.CreditDistribution(env, step=0),
            "final_credit_distributions": lending_metrics.CreditDistribution(env, step=-1),
            "recall": error_metrics.RecallMetric(
                env,
            ...
            "precision": error_metrics.PrecisionMetric(
                env,
                prediction_fn=lambda x: x.action,
                ground_truth_fn=lambda x: not x.state.will_default,
                stratify_fn=lambda x: str(x.state.group_id),
            ),
            "profit rate": value_tracking_metrics.ValueChange(env, state_var="bank_cash"),
        }
        ...
        metric_results = run_util.run_simulation(env, agent, metrics, self.num_steps, self.seed)
        report = {
            "environment": {
