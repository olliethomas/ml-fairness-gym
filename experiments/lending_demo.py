...
from experiments import lending, lending_plots
...
flags.DEFINE_bool(
    "equalize_opportunity", False, "If true, apply equality of opportunity constraints."
)
...
def main(argv):
    ...
    result = lending.Experiment(
        ...
        return_json=False,
        threshold_policy=(EQUALIZE_OPPORTUNITY if FLAGS.equalize_opportunity else MAXIMIZE_REWARD),
    ).run()
    ...
    lending_plots.plot_credit_distribution(
        ...
        "Initial",
        path=os.path.join(FLAGS.plots_directory, "initial_credit_distribution.png")
        if FLAGS.plots_directory
        else None,
        include_median=True,
        figure=fig,
    )
    ...
    lending_plots.plot_bars(
        ...
        path=os.path.join(FLAGS.plots_directory, "recall.png") if FLAGS.plots_directory else None,
        figure=fig,
    )
