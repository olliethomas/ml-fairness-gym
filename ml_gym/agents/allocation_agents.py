...
def _get_added_vector_features(observation, n, keys=None):
...
def _allocate_proportional_to_beliefs(rng, n_resource, beliefs):
  ...
  """
    ...
    if beliefs_total > 0:
        ...
        probabilities = [(1.0 / len(beliefs)) for _ in beliefs]
...
_AllocationAgentParamsBound = TypeVar("_AllocationAgentParamsBound", bound=AllocationAgentParams)
...
class AllocationAgent(core.Agent):
  ...
  """
    ...
    def __init__(self, action_space, reward_fn, observation_space, params=None):

        if reward_fn is None:
            reward_fn = rewards.NullReward()
        super(AllocationAgent, self).__init__(action_space, reward_fn, observation_space)
        if params is None:
    ...
    """
        ...
        if not self.observation_space.contains(observation):
            raise core.InvalidObservationError("Invalid ovservation: %s" % observation)
        if self.params.observation_adjustment_fn:
            observation = self.params.observation_adjustment_fn(self.rng, self.beliefs, observation)
...
class NaiveProbabilityMatchingAgent(AllocationAgent):
    ...
    def __init__(self, action_space, reward_fn, observation_space, params=None):
        ...
        super(NaiveProbabilityMatchingAgent, self).__init__(
            action_space, reward_fn, observation_space, params
        )
    ...
    """
        ...
        updated_beliefs = [beliefs[i] + features[i] - decay[i] for i in range(len(beliefs))]
    ...
    """
        ...
        contributions = np.array([(total - belief) / float(total) for belief in beliefs])
        ...
        for index in range(len(normalized_contributions)):
            if rng.random_sample() >= normalized_contributions[index]:
                for key in keys:
                    if (
                        len(normalized_contributions) != len(observation_copy[key])
                        or observation_copy[key].ndim != 1
                    ):
                        raise KeyError(
...
class _CensoredPoisson(model.GenericLikelihoodModel):
  ...
  """
    ...
    def fit(self, start_params=None, maxiter=10000, **kwds):
        ...
        return super(_CensoredPoisson, self).fit(start_params=start_params, maxiter=maxiter, **kwds)
    ...
    def _LL(self, data, rate):
        ...
        output_array += (observed_count < allocated_count) * stats.poisson.pmf(observed_count, rate)
...
_MLEProbabilityMatchingAgentParamsBound = TypeVar(
    "_MLEProbabilityMatchingAgentParamsBound", bound=MLEProbabilityMatchingAgentParams
)
...
class MLEProbabilityMatchingAgent(AllocationAgent):
  ...
  """
    ...
    def __init__(self, action_space, reward_fn, observation_space, params):
        ...
        super(MLEProbabilityMatchingAgent, self).__init__(
            action_space, reward_fn, observation_space, params
        )
    ...
    def _allocate(self, n_resource, beliefs):
        ...
        if self.rng.random_sample() < self.params.epsilon:
            ...
            self.last_allocation = _allocate_proportional_to_beliefs(self.rng, n_resource, beliefs)
    ...
    """
        ...
        for i_bin in range(self._n_bins):
            ...
            if self.params.burn_steps <= self.n_steps and self.n_steps % self.params.interval == 0:
                ll_model = _CensoredPoisson(np.array(self.data[i_bin][-self.params.window :]))
...
class MLEGreedyAgent(MLEProbabilityMatchingAgent):
  ...
  """
    ...
    def __init__(self, action_space, reward_fn, observation_space, params=None):
        ...
        super(MLEGreedyAgent, self).__init__(action_space, reward_fn, observation_space, params)
    ...
    """
        c_values = np.array(range(1, self.params.poisson_truncation_val), dtype=float).reshape(
            (self.params.poisson_truncation_val - 1, 1)
        )
        c_values_with0 = np.array(range(self.params.poisson_truncation_val), dtype=float).reshape(
            (self.params.poisson_truncation_val, 1)
        )
        alloc_vals = np.array(range(1, n_resource), dtype=float).reshape((n_resource - 1, 1))
        ...
        mins_over_c = np.concatenate((np.ones((1, n_resource - 1)), mins_over_c), axis=0)
        # can also switch the order of these concatenates depending on if min(v,c)/c
        # where v=0, c=0 should be 0 or one.
        mins_over_c = np.concatenate(
            (np.zeros((self.params.poisson_truncation_val, 1)), mins_over_c), axis=1
        )
    ...
    """
        ...
        if self.rng.binomial(1, self.params.epsilon):
            ...
            fi_table = self._construct_approx_fi_table(self._n_bins, beliefs, self._n_resource + 1)
            ...
            for bin_i in range(self._n_bins):
                current_allocation = np.zeros(self._n_bins, dtype=self.action_space.dtype)
                ...
                broadcast_shape = (self._n_resource + 1, len(rows), self._n_resource + 1)
                lower_bounds = np.argmax(
                    (
                        np.broadcast_to(fi_table[rows, :], broadcast_shape).T >= min_fi_table[bin_i]
                    ).T,
                    axis=2,
                )
                upper_bounds = (
                                ...
                                marginal_probs.append(
                                    (
                                        (
                                            ...
                                            - self._calculate_tail_probability(
                                                current_allocation[j], beliefs[j]
                                            )
                                        ),
                                        j,
                                    )
            ...
            if optimal_allocation is None:
                ...
                logging.warning("No allocation found for this alpha: %f", self.params.alpha)
                optimal_allocation = np.zeros(self._n_bins, dtype=self.action_space.dtype)
                raise gym.error.InvalidAction(
                    "Invalid action: %s with alpha %f" % (optimal_allocation, self.params.alpha)
                )
