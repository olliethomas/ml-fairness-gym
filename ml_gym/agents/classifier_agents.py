...
import collections
import logging
from typing import Any, Callable, List, Mapping, Optional, Text
...
import core
import params
from ml_gym.agents import threshold_policies
...
class TrainingCorpus(object):
    ...
    def get_features(self, stratify_by=None):
    ...
    def get_labels(self, stratify_by=None):
    ...
    """
        ...
        for example in self.examples:
            stratified_labels[tuple(example.observation.get(stratify_by))].append(example.label)
        return stratified_labels

    def get_weights(self, stratify_by=None):
    ...
    """
        ...
        for example in self.examples:
            stratified_weights[tuple(example.observation.get(stratify_by))].append(example.weight)
...
class ScoringAgentParams(core.Params):
    ...
    threshold_policy = attr.ib(default=threshold_policies.ThresholdPolicy.SINGLE_THRESHOLD)
    ...
    skip_retraining_fn = attr.ib(default=None)  # Optional[Callable[[Any, Dict[Text, Any]], bool]]
...
class ScoringAgent(core.Agent):
  ...
  """
    ...
    action_space = attr.ib(factory=lambda: gym.spaces.Discrete(2))  # type: gym.Space
    rng = attr.ib(factory=np.random.RandomState)  # type: np.random.RandomState

    global_threshold = attr.ib(default=0.0)
    ...
    def _act_impl(self, observation, reward, done):
        ...
        self._record_training_example(self._last_observation, self._last_action, reward)
    ...
    def _train(self):
        ...
        if self.params.use_propensity_score_weighting:
            ...
            raise NotImplementedError("propensity score weighting training is not implemented YET!")
    ...
    def _set_thresholds(self, training_corpus):
        ...
        if self.params.threshold_policy == threshold_policies.ThresholdPolicy.EQUALIZE_OPPORTUNITY:
            self.group_specific_thresholds = threshold_policies.equality_of_opportunity_thresholds(
                group_predictions=self._recursively_apply_score_transform(
                    training_corpus.get_features(stratify_by=self.params.group_key)
                ),
                group_labels=training_corpus.get_labels(stratify_by=self.params.group_key),
                group_weights=training_corpus.get_weights(stratify_by=self.params.group_key),
                cost_matrix=self.params.cost_matrix,
                rng=self.rng,
            )
    ...
    def debug_string(self):
        return "My thresholds are %s and %s" % (
...
class ClassifierAgent(ScoringAgent):
    ...
    _classifier = attr.ib(factory=lambda: linear_model.LogisticRegression(solver="lbfgs"))

    def _score_transform_update(self, training_corpus):
        try:
            self._classifier.fit(training_corpus.get_features(), training_corpus.get_labels())
        except ValueError:
            logging.warning(
    ...
    def _get_features(self, observation):
        return np.concatenate(
            [observation.get(feature_key) for feature_key in self.params.feature_keys], 0
        ).ravel()
