...
class NaiveJury(FixedJury):
  ...
  """
    ...
    def _get_default_features(self, observations):
        ...
        return [
            score
            for score, eligible in zip(
                observations["test_scores_y"], observations["selected_ground_truth"]
            )
    ...
    def _label_fn(self, observations):
        ...
        return [
            label
            for label in observations["selected_ground_truth"]
...
class RobustJury(NaiveJury):
  ...
  """
    ...
    def __init__(
        ...
        if burnin < 2:
            raise ValueError("This agent expects a longer burnin period, to work as expected.")

    # TODO (): Retain original observed scores in self._features.

    def _get_maximum_manipulated_features(self, observations):
        ...
        robust_features = []
        for score, eligible, group in zip(
            observations["test_scores_y"],
    ...
    def _get_max_allowed_score_change(self, observations):
