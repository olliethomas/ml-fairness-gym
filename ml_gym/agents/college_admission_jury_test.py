...
import params
import test_util
from ml_gym.agents import college_admission_jury
from ml_gym.environments import college_admission
...
class FixedJuryTest(absltest.TestCase):
    ...
    def test_agent_raises_episode_done_error(self):
        ...
        with self.assertRaises(core.EpisodeDoneError):
            agent.act(
                observation={"threshold": np.array(0.5), "epsilon_prob": np.array(0)}, done=True
            )
    ...
    def test_agent_produces_different_epsilon_with_epsilon_greedy(self):
        ...
        epsilon_probs.extend([float(agent.act(obs, done)["epsilon_prob"]) for _ in range(10)])
    ...
    def test_epsilon_prob_decays_as_expected(self):
        ...
        obs, _, done, _ = env.step(agent.initial_action())
        epsilon_probs = [float(agent.initial_action()["epsilon_prob"])]
        epsilon_probs.extend([float(agent.act(obs, done)["epsilon_prob"]) for _ in range(2)])
        self.assertTrue(np.all(np.isclose(epsilon_probs, [0.3, 0.0753, 0.0189], atol=1e-2)))
...
class NaiveJuryTest(absltest.TestCase):
    ...
    def test_agent_returns_same_threshold_till_burnin_and_then_change(self):
        ...
        test_util.run_test_simulation(env=env, agent=agent, num_steps=10, stackelberg=True)
    ...
    def test_agent_returns_correct_threshold(self):
        env = college_admission.CollegeAdmissionsEnv(
            user_params={
                "gaming": False,
                "subsidize": False,
                "noise_params": params.BoundedGaussian(max=0.3, min=0, sigma=0, mu=0.1),
                "feature_params": params.GMM(
...
class RobustJuryTest(absltest.TestCase):
    ...
    def test_features_manipulated_to_maximum_limit_with_no_control(self):
        env = college_admission.CollegeAdmissionsEnv(
            user_params={"num_applicants": 5, "gaming_control": np.inf, "group_cost": {0: 2, 1: 4}}
        )
        agent = college_admission_jury.RobustJury(
            action_space=env.action_space,
            observation_space=env.observation_space,
        ...
        self.assertEqual(agent._features, agent._get_maximum_manipulated_features(observations))

    def test_features_manipulated_to_maximum_limit_with_gaming_control(self):
        env = college_admission.CollegeAdmissionsEnv(
            user_params={"num_applicants": 5, "gaming_control": 0.3, "group_cost": {0: 2, 1: 4,}}
        )
        agent = college_admission_jury.RobustJury(
            action_space=env.action_space,
            observation_space=env.observation_space,
    ...
    def test_features_manipulated_to_maximum_limit_with_control_epsilon_greedy(self):
        env = college_admission.CollegeAdmissionsEnv(
            user_params={"num_applicants": 5, "gaming_control": 0.3, "group_cost": {0: 2, 1: 4,}}
        )
        agent = college_admission_jury.RobustJury(
    ...
    def test_features_manipulated_to_maximum_limit_no_control_epsilon_greedy(self):
        env = college_admission.CollegeAdmissionsEnv(
            user_params={"num_applicants": 5, "gaming_control": np.inf, "group_cost": {0: 2, 1: 4,}}
        )
        agent = college_admission_jury.RobustJury(
