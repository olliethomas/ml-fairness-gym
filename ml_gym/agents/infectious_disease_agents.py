...
def _infection_indicator(health_states, infected_state_index):
...
class _BaseAgent(core.Agent):
  ...
  """
    ...
    def __init__(self, action_space, reward_fn, observation_space, params):
        ...
        super(_BaseAgent, self).__init__(action_space, reward_fn, observation_space)
    ...
    """
        ...
        if not self.observation_space.contains(observation):
            raise core.InvalidObservationError("Invalid observation: %s." % observation)
...
class CentralityAgent(_BaseAgent):
    ...
    """
        ...
        assert list(observation["population_graph"].nodes()) == list(
...
class RandomAgent(_BaseAgent):
    ...
    """
        ...
        triage_scores = np.array([-infection * self.rng.rand() for infection in infections])
        max_treatments = len(self.action_space.nvec)
        return np.argsort(triage_scores)[:max_treatments]
...
class InteractiveAgent(_BaseAgent):
    ...
    def _triage(self, observation):
        ...
        infected_indices = [
            i
            for i, health_state in enumerate(observation["health_states"])
        ...
        while True:
            ...
            user_input = input("Please provide space-separated ids of people to treat: ")
            to_treat = [int(x) for x in user_input.split(" ")]
            if len(to_treat) != num_treatments:
                print(
                    ("You have %d treatments to give out, but selected %d people to " "treat.")
                    % (num_treatments, len(to_treat))
                )
            else:
                break
