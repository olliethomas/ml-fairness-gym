...
def set_up_and_observe(
    ...
    observation = {name: space.sample() for name, space in env.observable_state_vars.items()}
...
class InfectiousDiseaseAgentsTest(parameterized.TestCase):
    ...
    def test_infection_indicator_indicates_infection(self):
        self.assertCountEqual(
            [1, 0, 1], infectious_disease_agents._infection_indicator([2, 1, 2], 2)
        )
    ...
    def test_act_on_done_raises(self):
        ...
        initial_health_state = [0 for _ in range(50)]
        _, agent, observation = set_up_and_observe(
            population_graph=graph, initial_health_state=initial_health_state
        )
    ...
    def test_invalid_observation_raises(self):
        ...
        bad_observation = {
            key: val for key, val in good_observation.items() if key != "health_states"
        }
    ...
    @parameterized.parameters(
        (infectious_disease_agents.CentralityAgent,), (infectious_disease_agents.RandomAgent,)
    )
    def test_agents_choose_infected_people(self, agent_class):
        ...
        initial_health_state = [
            1 if index in infected_indices else 0 for index in range(population_size)
        ]
        ...
        observation = env._get_observable_state()
        action = agent.act(observation, False)
        self.assertCountEqual(infected_indices, action[: len(infected_indices)].tolist())
    ...
    def test_interactive_agent_treats_chosen_people(self):
        ...
        for index in infected_indices:
            self.assertLess(index, population_size)
        initial_health_state = [
            0 if index in infected_indices else 1 for index in range(population_size)
        ]
