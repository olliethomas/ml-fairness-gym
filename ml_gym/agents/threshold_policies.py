...
def _interpolate(x, low, high):
    ...
    assert low <= x <= high, ("x is not between [low, high]: Expected %s <= %s <=" " %s") % (
        low,
        x,
        high,
    )
...
def equality_of_opportunity_thresholds(
  ...
  """
    ...
    for group in groups:
        ...
        assert (
            len(group_labels[group]) == len(group_weights[group]) == len(group_predictions[group])
        )
    ...
    def negative_reward(tpr_target):
        ...
        for group in groups:
            ...
            for thresh_prob, threshold in _threshold_from_tpr(
                ...
                for weight, prediction in zip(group_weights[group], group_predictions[group]):
