...
class ThresholdPoliciesTest(absltest.TestCase):
    def test_equality_of_opportunity_holds(self):
        ...
        thresholds = threshold_policies.equality_of_opportunity_thresholds(
            group_predictions={"a": group_a_predictions, "b": group_b_predictions},
            group_labels={"a": group_a_labels, "b": group_b_labels},
            group_weights=None,
        ...
        group_a_recall = (
            (group_a_predictions > thresholds["a"].sample()) * group_a_labels
        ).sum() / group_a_labels.sum()
        group_b_recall = (
            (group_b_predictions > thresholds["b"].sample()) * group_b_labels
        ).sum() / group_b_labels.sum()
    ...
    def test_reward_is_maximized(self):
        ...
        thresholds = threshold_policies.equality_of_opportunity_thresholds(
            {"a": predictions}, {"a": labels}, group_weights=None, cost_matrix=COST_MATRIX, rng=rng
        )
        ...
        for _ in range(20):
            ...
            confusion = sklearn_metrics.confusion_matrix(labels, predictions >= challenge_threshold)
    ...
    def test_weighted_single_threshold(self):
        ...
        weights = np.array([3.0, 1.0, 2.0, 1.0, 1.0, 3.0])
        weights = weights / sum(weights)

        vanilla_cost_matrix = params.CostMatrix(tn=0.0, fp=-1.0, fn=0.0, tp=1.0)
