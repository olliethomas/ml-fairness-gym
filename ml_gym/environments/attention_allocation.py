...
import core
from ml_gym.spaces import multinomial
...
class Params(core.Params):
    ...
    incident_rates = attr.ib(factory=lambda: [4.0, 3.0])  # type: List[float]
    ...
    feature_means = attr.ib(factory=lambda: [1.0, 1.0])
...
def _get_location_features(params, rng, incidents_occurred):
  ...
  """
    ...
    feature_noise = rng.multivariate_normal(
        np.zeros_like(params.feature_means), params.feature_covariances, size=params.n_locations
    )
        ...
        for location_ind in range(params.n_locations):
            ...
            for _ in range(incidents_occurred[location_ind]):
                incidents_discovered = state.rng.binomial(
                    1, 1 - (np.power(params.miss_incident_prob[location_ind], unused_attention))
                )
    ...
    for location_ind in range(params.n_locations):
        ...
        if attention == 0:
            ...
            params.incident_rates[location_ind] = max(
                0.0, params.incident_rates[location_ind] - (params.dynamic_rate * attention)
            )

    state.location_features = _get_location_features(params, state.rng, incidents_occurred)
...
class LocationAllocationEnv(core.FairnessEnv):
  ...
  """
    ...
    def __init__(self, params=None):
        ...
        self.action_space = multinomial.Multinomial(params.n_locations, params.n_attention_units)

        assert params.n_locations == len(
            params.prior_incident_counts
        ...
        location_features_space = spaces.Box(
            low=-np.inf, high=np.inf, shape=(params.n_locations, n_features), dtype=np.float32
        )
    ...
    def _state_init(self, rng=None):
        ...
        self.state = State(
            ...
            location_features=np.zeros((n_locations, len(self.initial_params.feature_means))),
        )
    ...
    """
        incidents_occurred, reported_incidents = _sample_incidents(state.rng, state.params)
