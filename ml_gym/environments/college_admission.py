...
import copy
from typing import Any, Dict, List, Mapping, Optional, Text, Tuple, Union
...
class Params(core.Params):
    ...
    score_params = attr.ib(default=params.MinMax(max=1, min=0))  # type: params.MinMax
    # Stopping params.
    ...
    feature_params = attr.ib(
        default=params.GMM(mix_weight=[0.6, 0.4], mu=[0.8, 0.6], sigma=[0.1, 0.3])
    )  # type: params.GMM
    ...
    noise_params = attr.ib(
        default=params.BoundedGaussian(max=0.3, min=-0.3, mu=0.1, sigma=0.01)
    )  # type: params.BoundedGaussian

    # Scalar multiplier for cost (of a feature) per group.
    # Note: We codify B's disadvantage by requiring Cost A < Cost B.
    # pylint: disable=g-long-lambda
    group_cost = attr.ib(
        factory=lambda: {0: 0.8, 1: 1.2}  # pytype: disable=ignored-type-comment
  ...
  """
    ...
    """
        ...
        self.action_space = spaces.Dict(
            {
                ...
                "epsilon_prob": spaces.Box(low=0, high=1, dtype=np.float32, shape=()),
            }
        )  # type: spaces.Space
        ...
        self.observable_state_vars = {
            "test_scores_y": spaces.Box(
                low=env_params.score_params.min,
                high=env_params.score_params.max,
                dtype=np.float32,
                shape=(env_params.num_applicants,),
            ),
            "selected_applicants": spaces.MultiBinary(env_params.num_applicants),
            "selected_ground_truth": spaces.MultiDiscrete([3] * env_params.num_applicants),
            "applicant_groups": spaces.MultiBinary(env_params.num_applicants),
        }  # type: Dict[Text, spaces.Space]
        super(CollegeAdmissionsEnv, self).__init__(env_params)
        if env_params.gaming_control != np.inf and (
            env_params.gaming_control > 1 or env_params.gaming_control < 0
        ):
    ...
    def _state_init(self, rng=None):
        state = State(
            ...
            selected_ground_truth=np.array(
                [_UNSELECTED_INDICATOR] * self.initial_params.num_applicants
            ),
        )
    ...
    def _get_true_thresholds(self):
        ...
        return {
            group_id: mu[group_id] + (env_params.true_threshold_factor * mu[group_id])
    ...
    def _step_impl(self, state, action):
        ...
        state.test_scores_y, state.individual_burden = self._manipulate_features(state, action)
        # Select based on modified scores.
        state.selected_applicants, state.selected_ground_truth = self._select_candidates(
            state, action
        )
    ...
    def _manipulate_features(self, state, action):
        ...
        if env_params.gaming:
            for score, group_id in zip(state.test_scores_x, state.applicant_groups):
                if score < action["threshold"]:
                    ...
                    changed_score = np.clip(
                        action["threshold"] + noise,
                    ...
                    cost_of_manipulation = (
                        self._outcome(action["threshold"], changed_score)
                        - change_cost
                        - action["epsilon_prob"]
                    )

                    if (0 <= cost_of_manipulation <= 1) and (
                        change_cost < max_change_cost + _EPSILON_VALUE
                    ):
    ...
    def _select_candidates(self, state, action):
    ...
    """
        ...
        for changed_score in state.test_scores_y:
            predictions.append(
                self._epsilon_outcome(rng=state.rng, action=action, feature=changed_score)
            )
    ...
    def _sample_next_state_vars(self, state):
        ...
        state.original_test_scores, state.applicant_groups = self._sample_applicants(state.rng)
        if state.params.noisy_features:
            state.test_scores_x = [
                np.clip(score + self._add_noise(state.rng), score_params.min, score_params.max)
                for score in state.original_test_scores
            ]
        else:
            state.test_scores_x = copy.deepcopy(state.original_test_scores)

        state.true_eligible = [
            self._outcome(state.true_thresholds[group_id], score)
            for group_id, score in zip(state.applicant_groups, state.original_test_scores)
        ]
        return state
    ...
    """
        ...
        group_ids = []
        mixture_ids = rng.multinomial(
            env_params.num_applicants, env_params.feature_params.mix_weight
        )
        for group_id, count in enumerate(mixture_ids):
            ...
            original_test_scores.extend(
                np.clip(
                    rng.normal(
                        env_params.feature_params.mu[group_id],
                        env_params.feature_params.sigma[group_id],
                        count,
                    ),
                    env_params.score_params.min,
                    env_params.score_params.max,
    ...
    def _outcome(self, threshold, feature):
    ...
    def _epsilon_outcome(self, rng, action, feature):
    ...
    def _change_cost(self, original_score, changed_score, group_id):
        return self._cost_function(changed_score, group_id) - self._cost_function(
            original_score, group_id
        )
    ...
    """
        ...
        if noise_params.sigma > 0:
            ...
            if env_params.noise_dist == "gaussian":
                if (noise_params.min >= noise_params.mu) or (noise_params.mu >= noise_params.max):
                    raise ValueError(
                ...
                return np.clip(
                    noise_params.sigma * rng.randn() + noise_params.mu,
                    noise_params.min,
                    noise_params.max,
                )
            else:
