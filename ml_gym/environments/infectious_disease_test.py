...
from ml_gym.environments import infectious_disease
from ml_gym.spaces import graph as graph_space
...
class InfectiousDiseaseTest(absltest.TestCase):
    def test_disease_does_not_progress_without_contact(self):
        ...
        agent = random_agents.RandomAgent(env.action_space, lambda x: 0, env.observation_space)
    ...
    def test_disease_progresses_with_contact_si(self):
        ...
        env = infectious_disease.build_si_model(
            ...
            num_treatments=0,
            max_treatments=10,
            initial_health_state=[0 if i % 2 == 0 else 1 for i in range(graph.number_of_nodes())],
        )
        agent = random_agents.RandomAgent(env.action_space, lambda x: 0, env.observation_space)
    ...
    def test_disease_progresses_with_contact_during_burn_in(self):
        ...
        self.assertCountEqual(
            [1] + [0 for _ in range(population_size - 1)], env.state.health_states
        )

        # After burn-in, the infection frontier should have moved by burn_in steps.
        env.reset()
        self.assertEqual(
            [1 for _ in range(burn_in + 1)] + [0 for _ in range(population_size - burn_in - 1)],
    ...
    def test_disease_permanently_eradicated_if_whole_population_treated(self):
        ...
        initial_health_state = [1 if i in infected_indices else 0 for i in range(population_size)]
        ...
        env.seed(seed)
        self.assertGreater(num_in_health_state(env.state, env.state_name_to_index["infected"]), 0)

        # Treat everyone and ensure the disease is gone.
        total_treatment = np.arange(population_size)
        env.step(total_treatment)
        self.assertEqual(0, num_in_health_state(env.state, env.state_name_to_index["infected"]))
    ...
    def test_last_population_used_for_infection_propagation(self):
        ...
        self.assertEqual(env.state_name_to_index["susceptible"], env.state.health_states[2])
    ...
    def test_one_transition_per_step_in_seir(self):
        ...
        initial_health_state = [
            2 if i in initially_infected_nodes else 0 for i in range(population_size)
        ]
        ...
        self.assertCountEqual(
            [
                env.state_name_to_index["exposed"]
                for i in range(population_size)
                if i not in initially_infected_nodes
            ],
            [
                env.state.health_states[i]
                for i in range(population_size)
                if i not in initially_infected_nodes
            ],
        )
        self.assertCountEqual(
            [
                env.state_name_to_index["recovered"]
                for i in range(population_size)
    ...
    def test_graph_space_contains_contact_network(self):
        ...
        graph.add_edges_from(complete_graph_edge_list(population_size))
        env = infectious_disease.build_si_model(
            population_graph=graph, infection_probability=1.0, num_treatments=0, max_treatments=10
        )
    ...
    def test_population_transforming_treatments_seir(self):
        ...
        for i in range(4):
            ...
            env.step(np.arange(population_size))
            self.assertCountEqual([i for _ in range(population_size)], env.state.health_states)
