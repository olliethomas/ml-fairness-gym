...
class _ApplicantSampler(core.StateUpdater):
    ...
    def update(self, state, action):
        ...
        state.applicant_features = np.clip(
            new_applicant.features, params.min_observation, params.max_observation
        )
  ...
  """
    ...
    """
        ...
        if mode == "human":
            ...
            plt.subplot(1, 2, 2)
            plt.title("Cash")
            plt.plot([state.bank_cash for state, _ in self.history] + [self.state.bank_cash])
...
class _CreditShift(core.StateUpdater):
    ...
    """
        ...
        assert state.group_id == np.argmax(state.group), "Group id %s. group %s" % (
            state.group_id,
            np.argmax(state.group),
        )

        # Cast to list so we can mutate it.
        cluster_probs = list(params.applicant_distribution.components[group_id].weights)

        rng = np.random.RandomState()
        for _ in range(10):
            group = params.applicant_distribution.components[group_id].sample(rng).group
            assert np.array_equal(group, state.group), (
                ...
                "Component[%d]: %s"
                % (group, state.group, group_id, params.applicant_distribution.components[group_id])
            )
        ...
        assert np.abs(np.sum(cluster_probs) - 1) < 1e-6, "Cluster probs must sum to 1."
        assert all([prob >= 0 for prob in cluster_probs]), "Cluster probs must be non-negative"

        state.params.applicant_distribution.components[group_id].weights = cluster_probs
