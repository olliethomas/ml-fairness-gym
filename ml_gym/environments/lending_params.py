...
def _gmm_applicant_builder(mean, intercluster_vec, default_likelihoods, group):
...
def _rotated_gmm_applicant_builder(
    mean, intercluster_vec, default_likelihoods, degrees, group_likelihoods
):
...
def _credit_cluster_builder(group_membership, cluster_probs, success_probs):
...
class Params(core.Params):
    ...
    min_observation = attr.ib(default=-2.0)  # type: float
    max_observation = attr.ib(default=2.0)  # type: float

    # Loan constants.
    loan_amount = attr.ib(default=1.0)  # type: float
...
class DelayedImpactParams(Params):
  ...
  """
    ...
    applicant_distribution = attr.ib(
        factory=two_group_credit_clusters
    )  # type: distributions.Distribution
    num_groups = attr.ib(default=2)  # type: int
