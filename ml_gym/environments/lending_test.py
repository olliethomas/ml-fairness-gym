...
import test_util
from ml_gym.environments import lending


class GroupsLoanTest(parameterized.TestCase):
    @parameterized.parameters(
        (lending.SimpleLoans,), (lending.DifferentialExpressionEnv,), (lending.DelayedImpactEnv,)
    )
    ...
    @parameterized.parameters((lending.DifferentialExpressionEnv,), (lending.DelayedImpactEnv,))
    ...
    def test_credit_score_distributions_change(self):
        ...
        self.assertLess(
            get_cluster_probs(state.params)[3], get_cluster_probs(env.initial_params)[3]
        )
        self.assertGreater(
            get_cluster_probs(state.params)[2], get_cluster_probs(env.initial_params)[2]
        )
    ...
    @parameterized.parameters((lending.SimpleLoans,), (lending.DifferentialExpressionEnv,))
