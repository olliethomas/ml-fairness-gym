...
class DistributionComparisonMetric(core.Metric):
  ...
  """
    ...
    def __init__(self, environment, state_variable, window=100, realign_fn=None):
    ...
    """
        ...
        relevant_state_items = [getattr(state, self.state_variable) for state in states]
        ...
        if not np.all([np.array(state_item).ndim == 1 for state_item in relevant_state_items]):
            raise ValueError(
        ...
        action_distribution_normalized = action_distribution / np.sum(action_distribution)

        state_distribution = np.sum(relevant_state_items, axis=0) / float(self.window)
        state_distribution_normalized = state_distribution / np.sum(state_distribution)

        distribution_distance = np.linalg.norm(
            state_distribution_normalized - action_distribution_normalized
        )

        return (
            state_distribution_normalized,
