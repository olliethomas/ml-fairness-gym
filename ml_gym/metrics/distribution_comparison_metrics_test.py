...
from ml_gym.metrics import distribution_comparison_metrics
...
class DistributionComparisonMetricsTest(absltest.TestCase):
    def test_measure(self):
        ...
        self.assertTrue(np.all(np.isclose(state_dist, expected_state_dist, atol=0.05)))
        self.assertTrue(np.all(np.isclose(action_dist, expected_action_dist, atol=0.05)))
    ...
    def test_error_on_scalar(self):
        ...
        metric = distribution_comparison_metrics.DistributionComparisonMetric(env, "x", 100)
