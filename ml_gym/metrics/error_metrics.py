...
class ConfusionMatrix(object):
    ...
    tp = attr.ib(default=0.0)  # type: float
    tn = attr.ib(default=0.0)  # type: float
    fp = attr.ib(default=0.0)  # type: float
    fn = attr.ib(default=0.0)  # type: float
...
class AccuracyMetric(core.Metric):
    ...
    def __init__(self, env, numerator_fn, denominator_fn=None, stratify_fn=None, realign_fn=None):
    ...
    """
        ...
        for step in history:
            ...
            assert len(correct_predictions) == len(predictions_to_keep) == len(stratification), (
                "Expected stratification, correct_predictions and predictions_to_keep"
                " to have the same length, but found %d, %d, %d respectively."
                % (len(stratification), len(correct_predictions), len(predictions_to_keep))
            )
            for correct_prediction, to_keep, stratum in zip(
                correct_predictions, predictions_to_keep, stratification
            ):
                if to_keep:
...
class ConfusionMetric(core.Metric):
    ...
    def __init__(self, env, prediction_fn, ground_truth_fn, stratify_fn=None, realign_fn=None):
...
class CostedConfusionMetric(ConfusionMetric):
    ...
    """
        super(CostedConfusionMetric, self).__init__(
            env, prediction_fn, ground_truth_fn, stratify_fn, realign_fn
        )
...
class PrecisionMetric(ConfusionMetric):
    ...
    """
        ...
        return {stratum: confusion.precision for stratum, confusion in result.items()}
