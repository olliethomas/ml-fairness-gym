...
import rewards
import test_util
from ml_gym.metrics import error_metrics
...
class ErrorMetricTest(absltest.TestCase):
    ...
    def test_stratified_accuracy_metric_correct_atomic_prediction(self):
        ...
        measurement = test_util.run_test_simulation(env=env, agent=None, metric=metric)
    ...
    def test_recall_with_zero_denominator(self):
        ...
        metric = error_metrics.RecallMetric(
            env=env, prediction_fn=lambda x: 0, ground_truth_fn=lambda x: 0, stratify_fn=lambda x: 1
        )
    ...
    def test_cost_metric_correct_for_atomic_prediction_rule(self):
        ...
        measurement = test_util.run_test_simulation(env=env, agent=None, metric=cost_metric)
    ...
    def test_confusion_as_array_and_cost_as_array_line_up(self):
        ...
        confusion_matrix = error_metrics.ConfusionMatrix(tp=1, tn=2, fp=3, fn=4).as_array()
