...
from ml_gym.metrics import lending_metrics
...
class CreditDistributionTest(absltest.TestCase):
    def test_final_credit_distribution_metric_can_interact_with_lending(self):
        ...
        test_util.run_test_simulation(env=env, metric=[final_distribution, initial_distribution])
    ...
    def test_measure_distribution_change_measurement(self):
        ...
        clusters = distributions.Mixture(
            components=[
                lending_params._credit_cluster_builder(
                    group_membership=[1, 0], cluster_probs=[0.1, 0.9], success_probs=[1.0, 0.0]
                )(),
                lending_params._credit_cluster_builder(
                    group_membership=[0, 1], cluster_probs=[0.8, 0.2], success_probs=[1.0, 0.0]
                )(),
            ],
            weights=(0.5, 0.5),
        )
        ...
        self.assertEqual({"0": [0.1, 0.9], "1": [0.8, 0.2]}, initial_distribution.measure(env))
        self.assertNotEqual({"0": [0.1, 0.9], "1": [0.8, 0.2]}, final_distribution.measure(env))
