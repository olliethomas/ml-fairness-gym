...
import rewards
import test_util
from ml_gym.environments import attention_allocation, lending
from ml_gym.metrics import value_tracking_metrics
...
class AggregatingMetricsTest(absltest.TestCase):
  ...
  """
    ...
    """
        ...
        self.assertSequenceAlmostEqual(sorted(measurement.values()), [-20, 15], delta=1e-4)
    ...
    """
        ...
        logging.info("Measurement result: %s.", measurement)
        self.assertSequenceAlmostEqual(sorted(measurement.values()), [-2, 1.5], delta=1e-4)
    ...
    """
        ...
        logging.info("Measurement result: %s.", measurement)
        self.assertSequenceAlmostEqual(sorted(measurement.values()), [-0.4, 0.3], delta=1e-4)
    ...
    Expected values:
      ...
      group 1 = sum([1] * 10 for 5 steps) = 50
    """
        env, metric = _setup_test_simulation(dim=10, calc_mean=False, modifier_fn=None)
        measurement = test_util.run_test_simulation(
            env=env, agent=None, metric=metric, num_steps=10
        )
        logging.info("Measurement result: %s.", measurement)
        self.assertSequenceAlmostEqual(sorted(measurement.values()), [0, 50], delta=1e-4)

    def test_aggregate_metric_give_correct_result_for_atomic_value_no_modifier(self):
    ...
    """
        env, metric = _setup_test_simulation(dim=1, calc_mean=False, modifier_fn=None)
        measurement = test_util.run_test_simulation(
            env=env, agent=None, metric=metric, num_steps=10
        )
        logging.info("Measurement result: %s.", measurement)
        self.assertSequenceAlmostEqual(sorted(measurement.values()), [0, 5], delta=1e-4)
...
class SummingMetricsTest(absltest.TestCase):
    def test_summing_metric_give_correct_sum_dummy_env(self):
        ...
        metric = value_tracking_metrics.SummingMetric(env=env, selection_fn=_selection_fn)
        measurement = test_util.run_test_simulation(env, agent=None, metric=metric, seed=0)
...
class ValueChangeTest(absltest.TestCase):
    def test_value_change_measures_correctly_unnormalized(self):
        env = IncreasingEnv()
        metric = value_tracking_metrics.ValueChange(env, "x", normalize_by_steps=False)
    ...
    def test_value_change_measures_correctly_normalized(self):
        env = IncreasingEnv()
        metric = value_tracking_metrics.ValueChange(env, "x", normalize_by_steps=True)
        for _ in range(17):
