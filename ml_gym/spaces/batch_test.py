...
class BatchTest(absltest.TestCase):
    def test_batches_are_contained_in_space(self):
        base_space = spaces.Tuple(
            (spaces.Discrete(2), spaces.Box(low=-1.0, high=1.0, shape=(3,), dtype=np.float32))
        )
        ...
        for batch_size in [0, 1, 5, 20]:
            self.assertTrue(batch_space.contains([base_space.sample() for _ in range(batch_size)]))
