...
class GraphSpace(gym.Space):
  ...
  """
    ...
    def contains(self, item):
        return isinstance(item, nx.Graph) and item.number_of_nodes() == self.num_nodes

    def sample(self):
        return nx.fast_gnp_random_graph(self.num_nodes, self.p, directed=self.directed)

    def __repr__(self):
        return "Graph (%d, %.4f, %s)" % (self.num_nodes, self.p, self.directed)
