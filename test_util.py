...
from ml_gym.spaces import batch
...
class DummyEnv(core.FairnessEnv):
    ...
    observable_state_vars = {"x": gym.spaces.Box(low=0, high=1, shape=(1,), dtype=np.float32)}
    ...
    def _state_init(self, rng=None):
        self.state = DummyState(
            rng=rng or np.random.RandomState(), x=np.array([0.5]), params=self.initial_params
        )
...
def run_test_simulation(
  ...
  """
    env, agent, metric = setup_test_simulation(env=env, agent=agent, metric=metric)
    ...
    simulator = run_util.run_stackelberg_simulation if stackelberg else run_util.run_simulation
    ...
    if check_reproducibility:
        ...
        simulator(env, cloned_agent, cloned_metric, num_steps, seed=seed, agent_seed=seed)
        ...
        for step, ((state_a, action_a), (state_b, action_b)) in enumerate(
            zip(base_history, cloned_history)
        ):
            if state_a != state_b:
                raise core.NotReproducibleError(
                    "Step %d. State mismatch: %s vs %s" % (step, state_a, state_b)
                )
            if action_a != action_b:
                raise core.NotReproducibleError(
                    "Step %d. Action mismatch: %s vs %s" % (step, action_a, action_b)
                )
...
DummyAgent = gin.external_configurable(random_agents.RandomAgent, name="test_util.DummyAgent")
